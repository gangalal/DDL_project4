...
// GPPUA = 0x0C;
// GPPUB = 0x0D;
// Also must include interrupt registers and conditions for interrupt flag in pins with switches
...
void manipulation()
{
    ...
    int tempInF = convertToF(tempInC);
    if (((switchStatus >> 7) & 0x01) == 0) // read Temperature in C
    {
        int digitA = readTemperature() / 10;
        int digitB = readTemperature() % 10;
        writeToExpander(digitA, digitB);
    }
    else if (((switchStatus >> 7) & 0x01) == 1) // read Temperature in F
    {
